...
- 当前未发现专门覆盖删除回收与云端备份的现成 E2E 用例

建议新增的 E2E 场景：

- 话题列表中删除默认临时工区话题，验证提示文案、删除后列表刷新、重新进入不存在
- 工作区文件树删除文件/文件夹，验证确认框、删除后树刷新
- 设置页云备份配置、连接测试、手动备份成功提示
- 恢复成功后在 dev 模式显示“手动重启”提示
- 恢复失败时显示 inline error，不关闭弹窗
- 跨平台备份 manifest 导致恢复成功后的 warning 呈现
