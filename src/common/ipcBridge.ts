...
export const backup = {
  ...
  checkRemoteConnection: bridge.buildProvider<
    IBridgeResponse<IBackupConnectionResult>,
    { settings: ICloudBackupSettings }
  >('backup.check-remote-connection'),
  runRemoteBackup: bridge.buildProvider<
    IBridgeResponse<IRemoteBackupFile>,
    { settings: ICloudBackupSettings; fileName?: string; automatic?: boolean; requestId?: string }
  >('backup.run-remote-backup'),
  cancelTask: bridge.buildProvider<IBridgeResponse<{ canceled: boolean }>, { requestId?: string } | undefined>(
    'backup.cancel-task'
  ),
  listRemotePackages: bridge.buildProvider<IBridgeResponse<IRemoteBackupFile[]>, { settings: ICloudBackupSettings }>(
    'backup.list-remote-packages'
  ),
  restoreRemotePackage: bridge.buildProvider<
    IBridgeResponse<{ fileName: string; restartRequired: boolean; manifest?: IBackupManifest }>,
    { settings: ICloudBackupSettings; fileName: string; requestId?: string }
  >('backup.restore-remote-package'),
