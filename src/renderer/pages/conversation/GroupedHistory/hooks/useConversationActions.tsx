...
export const useConversationActions = ({
  ...
  const buildDeleteReminderContent = useCallback(
    (workspaceMode: TWorkspaceSource | null) => {
      if (!workspaceMode) {
        return (
          <div
            data-testid='conversation-delete-dialog-content'
            className='text-14px leading-6 text-[var(--color-text-2)]'
          >
      ...
      const reminderMeta = {
        temporary: {
          ...
          accentClass:
            'bg-[rgba(var(--warning-6),0.12)] text-[rgb(var(--warning-6))] border-[rgba(var(--warning-6),0.18)]',
        },
        migrated: {
          label: t('conversation.history.deleteImpactMigrated'),
          detail: t('conversation.history.deleteImpactMigratedDetail'),
          accentClass:
            'bg-[rgba(var(--primary-6),0.10)] text-[rgb(var(--primary-6))] border-[rgba(var(--primary-6),0.18)]',
        },
        manual: {
      ...
      return (
        <div data-testid='conversation-delete-dialog-content' className='space-y-12px'>
          <div className='text-14px leading-6 text-[var(--color-text-2)]'>
            {t('conversation.history.deleteConfirm')}
          </div>
          <div className='rounded-12px bg-[var(--color-fill-1)] p-12px'>
            <div className='text-12px font-500 leading-5 text-[var(--color-text-3)]'>
              {t('conversation.history.deleteImpactTitle')}
            </div>
            <div
              data-testid={`conversation-delete-impact-${workspaceMode}`}
              className={`mt-8px inline-flex items-center rounded-full border px-8px py-4px text-12px font-600 leading-none ${reminderMeta.accentClass}`}
            >
              {reminderMeta.label}
            </div>
  ...
  const buildBatchDeleteReminderContent = useCallback(
    (selectedConversations: TChatConversation[]) => {
      ...
      if (reminderItems.length === 0) {
        return (
          <div
            data-testid='conversation-batch-delete-dialog-content'
            className='text-14px leading-6 text-[var(--color-text-2)]'
          >
          ...
          <div className='rounded-12px bg-[var(--color-fill-1)] p-12px'>
            ...
            <div className='mt-8px space-y-8px'>
              {reminderItems.map((item) => (
                <div
                  key={item.key}
                  data-testid={`conversation-batch-delete-impact-${item.key}`}
                  className='rounded-10px bg-[var(--color-bg-1)] p-10px'
                >
                  <div
                    className={`inline-flex items-center rounded-full border px-8px py-4px text-12px font-600 leading-none ${item.accentClass}`}
                  >
  ...
  const handleBatchDelete = useCallback(() => {
    ...
    buildBatchDeleteReminderContent,
    conversations,
    onBatchModeChange,
    removeConversation,
    selectedConversationIds,
