...
const CloudBackupRestoreProgressModal: React.FC<CloudBackupRestoreProgressModalProps> = ({
  visible,
  fileName,
  requestId,
  taskEvent,
  ...
  const resolvedError =
    errorMessage ||
    (activeEvent?.phase === 'error' ? formatCloudBackupErrorMessage(activeEvent.errorCode, activeEvent.message) : null);
  ...
  const shouldShowCrossPlatformWarning = Boolean(
    manifest?.sourcePlatform && currentPlatform && manifest.sourcePlatform !== currentPlatform
  );
  const canCancel =
    Boolean(requestId) && !isError && !restartPending && (!activeEvent || activeEvent.cancellable === true);
  ...
  return (
    <Modal
      ...
      title={t('settings.backup.restoreProgressTitle')}
      visible={visible}
      focusLock={false}
      autoFocus={false}
      maskClosable={isError}
      closable={isError}
      escToExit={isError}
      onCancel={isError ? onClose : undefined}
      footer={null}
              ...
              <div className='flex items-start gap-10px'>
                ...
                <div className='min-w-0 flex-1'>
                  <div className='text-15px font-600 text-[var(--color-text-1)]'>
                    {t('settings.backup.restoreErrorTitle')}
                  </div>
          ...
          <>
            <div className='rounded-16px border border-solid border-[var(--color-success-light-3)] bg-[var(--color-success-light-1)] px-16px py-16px'>
              <div className='flex items-start gap-10px'>
                ...
                <div className='min-w-0 flex-1'>
                  <div className='text-16px font-600 text-[var(--color-text-1)]'>
                    {t('settings.backup.restoreSuccessTitle')}
                  </div>
                  <div className='mt-6px text-13px leading-5 text-[var(--color-text-3)]'>
                    {isDevManualRestartMode
                      ? t('settings.backup.restoreRestartManual')
                      : t('settings.backup.restoreRestartCountdown', { count: restartCountdown ?? 0 })}
                  </div>
                </div>
              </div>
            </div>
