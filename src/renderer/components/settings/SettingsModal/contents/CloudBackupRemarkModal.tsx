...
const CloudBackupRemarkModal: React.FC<CloudBackupRemarkModalProps> = ({
  visible,
  settings,
  taskEvent,
  onClose,
  ...
  const isRunning =
    Boolean(activeRequestId) && (!activeEvent || (activeEvent.phase !== 'success' && activeEvent.phase !== 'error'));
    ...
    Modal.confirm({
      ...
      content: (
        <div className='space-y-8px'>
          <div className='text-14px leading-6 text-[var(--color-text-1)]'>
            {t('settings.backup.cancelConfirmContent')}
          </div>
        ...
        {!isSuccess && (
          <div className='space-y-8px'>
            ...
            <Input
              ...
              placeholder={t('settings.backup.fileNamePlaceholder')}
              disabled={loadingSuggestedName || isRunning}
              spellCheck={false}
            />
          </div>
        )}
          ...
          <>
            ...
            <div className='grid gap-10px md:grid-cols-2'>
              <div className='rounded-12px border border-solid border-[var(--color-border-2)] bg-[var(--fill-1)] px-12px py-10px'>
                ...
                <div className='mt-4px text-13px font-600 text-[var(--color-text-1)]'>
              ...
              <div className='rounded-12px border border-solid border-[var(--color-border-2)] bg-[var(--fill-1)] px-12px py-10px'>
                ...
                <div className='mt-4px break-all text-13px font-600 text-[var(--color-text-1)]'>
                  {settings.activeProvider === 'nutstore' ? settings.nutstore.remotePath : settings.webdav.remotePath}
                </div>
              </div>
            </div>
          </>
              ...
              <div className='flex flex-col gap-14px'>
                <div className='flex flex-wrap items-start justify-between gap-12px'>
                  ...
                  <div className='text-12px text-[var(--color-text-3)]'>
                    {t('settings.backup.elapsedTime', { time: formatElapsedTime(elapsedSeconds) })}
                  </div>
                </div>
          ...
          <div className='space-y-16px'>
            <div className='rounded-16px border border-solid border-[var(--color-success-light-3)] bg-[var(--color-success-light-1)] px-16px py-18px'>
              <div className='flex items-start gap-10px'>
                ...
                <div className='min-w-0 flex-1'>
                  <div className='text-17px font-600 text-[var(--color-text-1)]'>
                    {t('settings.backup.successTitle')}
                  </div>
            ...
            <div className='grid gap-10px md:grid-cols-2'>
              ...
              <div className='rounded-12px border border-solid border-[var(--color-border-2)] bg-[var(--fill-1)] px-12px py-10px'>
                ...
                <div className='mt-4px text-13px font-600 text-[var(--color-text-1)]'>
                  {activeEvent ? formatter.format(new Date(activeEvent.timestamp)) : '--'}
                </div>
              </div>
              <div className='rounded-12px border border-solid border-[var(--color-border-2)] bg-[var(--fill-1)] px-12px py-10px'>
                <div className='text-12px text-[var(--color-text-3)]'>
                  {t('settings.backup.includeDefaultWorkspaceFiles')}
                </div>
                <div className='mt-4px text-13px font-600 text-[var(--color-text-1)]'>
                  {settings.includeDefaultWorkspaceFiles
                    ? t('settings.backup.included')
                    : t('settings.backup.notIncluded')}
                </div>
              </div>
              <div className='rounded-12px border border-solid border-[var(--color-border-2)] bg-[var(--fill-1)] px-12px py-10px'>
                <div className='text-12px text-[var(--color-text-3)]'>{t('settings.backup.successSize')}</div>
                <div className='mt-4px text-13px font-600 text-[var(--color-text-1)]'>
                  {formatFileSize(activeEvent?.fileSize)}
                </div>
              </div>
            </div>
          </div>
