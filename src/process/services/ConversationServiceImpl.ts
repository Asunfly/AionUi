...
import { deleteConversationData } from './conversation/deleteConversationData';
...
export class ConversationServiceImpl implements IConversationService {
  ...
  async deleteConversation(id: string): Promise<void> {
    ...
    const deleted = await deleteConversationData(id);
    if (!deleted) {
  ...
  async createWithMigration(params: MigrateConversationParams): Promise<TChatConversation> {
    ...
    if (sourceConversationId) {
      ...
      if (sourceMsgs.total === newMsgs.total) {
        const deleted = await deleteConversationData(sourceConversationId);
        if (!deleted) {
          console.warn(
            `[ConversationServiceImpl] Failed to clean up migrated source conversation ${sourceConversationId}`
          );
        }
      } else {
