...
const conversationHistoryProxy = (options: typeof _chatMessageFile, dir: string) => {
  const getConversationHistoryFilePath = (conversation_id: string) =>
  ...
  return {
    ...
    async remove(key: string) {
      await Promise.allSettled([
        options.remove(key as keyof ConversationHistoryData),
        fs.rm(getConversationHistoryFilePath(key), { recursive: true, force: true }),
      ]);
    },
  };
};
