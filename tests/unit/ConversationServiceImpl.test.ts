...
const deleteConversationDataMock = vi.hoisted(() => vi.fn(async () => true));
vi.mock('../../src/process/services/conversation/deleteConversationData', () => ({
  deleteConversationData: deleteConversationDataMock,
}));
...
describe('ConversationServiceImpl.deleteConversation', () => {
  it('calls deleteConversationData after cron cleanup', async () => {
    const repo = makeRepo();
    const svc = new ConversationServiceImpl(repo);
    await svc.deleteConversation('c1');
    expect(deleteConversationDataMock).toHaveBeenCalledWith('c1');
  });
});
...
describe('ConversationServiceImpl.createWithMigration', () => {
  ...
  it('copies messages from source conversation', async () => {
    ...
    expect(deleteConversationDataMock).toHaveBeenCalledWith('src');
  });
});
