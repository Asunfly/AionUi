...
import {
  ...
  mapManagedWorkspacePath,
  normalizeManagedWorkspaceRelativePath,
  remapConversationExtraPaths,
} from '../../src/process/services/backup/workspaceBackup';
...
describe('workspace backup helpers', () => {
  ...
  it('maps managed workspace paths to the current machine work directory across platforms', () => {
    expect(
      mapManagedWorkspacePath(
        'C:\\AionUi\\work\\gemini-temp-1',
        'C:\\AionUi\\work',
        '/home/demo/.config/AionUi/work',
        'win32',
        false
      )
    ).toBe(path.posix.join('/home/demo/.config/AionUi/work', 'gemini-temp-1'));
    expect(
      mapManagedWorkspacePath(
        '/Users/demo/aionui/work/codex-temp-2',
        '/Users/demo/aionui/work',
        'D:\\AionUi\\work',
        'darwin'
      )
    ).toBe('D:\\AionUi\\work\\codex-temp-2');
    expect(
      mapManagedWorkspacePath(
        'D:\\external-project',
        'C:\\AionUi\\work',
  ...
  it('remaps conversation workspace fields and openclaw runtime validation together', () => {
    ...
    expect(result.value).toEqual({
      workspace: path.posix.join('/home/demo/.config/AionUi/work', 'openclaw-temp-1'),
      customWorkspace: false,
      runtimeValidation: {
        expectedWorkspace: path.posix.join('/home/demo/.config/AionUi/work', 'openclaw-temp-1'),
        expectedBackend: 'openclaw-gateway',
      },
    });
  });
